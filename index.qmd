# Introduction 
The `finmetrika-ml` library is a machine learning library for practical projects, predominantly for the financial industry. 

The library is organized as follows:
```bash
finmetrika_ml
├── data
    ├── data_processing.py
    └── vizualization.py
├── model
    ├── training.py
    ├── evaluation.py
    └── metrics.py
└── utils.py
```

## Installation

To install use the `pip` command in your virtual environment:
```bash
pip install finmetrika_ml
```

## Create Jupyter notebook from templates

To create a Jupyter notebook from template run the following command in terminal:
```bash
fm_create_nb NOTEBOOK-PATH/NOTEBOOK-NAME.ipynb
```
and replace `NOTEBOOK-PATH` with the path directory where you wish the notebook to be saved and `NOTEBOOK-NAME` with your desired name choice.

